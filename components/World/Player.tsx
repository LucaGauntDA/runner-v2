...
export const Player: React.FC = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Movement mapping (Arrow keys + WASD)
  ...
  useFrame((state, delta) => {
    ...
    if (isJumping.current) {
        ...
        if (jumpsPerformed.current === 2 && bodyRef.current) {
             spinRotation.current -= safeDelta * 18;
    ...
    if (!isJumping.current) {
        ...
        const jumpPoseSpeed = safeDelta * 12;
    ...
    if (shadowRef.current) {
        ...
        const scale = Math.max(0.2, 1 - (height / 3.0) * 0.6); 
        ...
        if (material && !Array.isArray(material)) {
            material.opacity = Math.max(0.05, 0.3 - (height / 3.0) * 0.25);
        }
    }
